...
class Hapt2Ignatov:
    """
    The shapes of sensor readings stored in 'x_train.npy',"y_train.npy","fea_train.npy" are
    (7767, 128, 6), (7767,) and (7767,40) respectively.
    The input shape of Ignatov model should be N x D(6) x 1 x S(usually128)

    """

    def __call__(self, _sample):
        input, label = _sample["input"], _sample["label"]
        reading, fea_vec = input
        reading = np.swapaxes(reading, 0, 1)
        reading = reading[:, np.newaxis, :]
        return {"input": (reading, fea_vec), "label": label}
